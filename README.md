...
如果你只想直接使用该插件，无需自己配置环境编译，请按照以下步骤操作：
...
3. 打开电脑的文件资源管理器，在地址栏输入并进入以下路径：
   `%APPDATA%\Salt Player for Windows\workshop\plugins\`
4. 将下载的 `.zip` 压缩包**直接放入**该文件夹中（**无需解压**）。
5. 完全重启 Salt Player for Windows，进入【设置】 -> 【创意工坊】 -> 【模组管理】，确保本插件已启用。
...
```bash
# 克隆仓库
git clone https://github.com/你的用户名/spw-fixed-shuffle-plugin.git
